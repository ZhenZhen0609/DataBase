...
void MainWindow::onInsertRecord()
{
    ...
    // 构建提示字符串
    QStringList fieldHints;
    for (const Field &f : schema.fields) fieldHints << f.name;
    QString hint = QString("按顺序输入字段值（逗号分隔）\n字段: %1").arg(fieldHints.join(", "));

    QString input = QInputDialog::getText(this, "插入记录", hint);
    if (input.trimmed().isEmpty()) return;

    QStringList values = input.split(',');
    if (values.size() != schema.fields.size()) {
        log(QString("[UI] 字段数量不匹配，期望 %1 个").arg(schema.fields.size()));
        return;
    }
    ...
    for (int i = 0; i < schema.fields.size(); ++i) {
        ...
        if (f.type == FieldType::INT)
            record[f.name] = val.toInt();
        else if (f.type == FieldType::DOUBLE)
            record[f.name] = val.toDouble();
        else if (f.type == FieldType::BOOLEAN)
            record[f.name] = (val.toLower() == "true");
        else
            record[f.name] = val;
    }
...
void MainWindow::showDataTable(const QString &username, const QString &dbName, const QString &tableName)
{
    ...
    QJsonArray records = r.data.toJsonArray();
    ...
    for (const QJsonValue &v : records) {
        for (const QString &k : v.toObject().keys())
            if (!cols.contains(k) && k != "_created_at") cols << k;
    }
    cols << "_created_at"; // 时间戳放最后
    ...
    for (int row = 0; row < records.size(); ++row) {
        ...
        for (int col = 0; col < cols.size(); ++col) {
            ...
            QString text = val.isDouble() ? QString::number(val.toDouble()) : val.toString();
